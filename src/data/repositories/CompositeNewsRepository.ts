...
export class CompositeNewsRepository implements NewsRepository {
  ...
  public async getTopHeadlines(
    ...
    const targets = params?.provider
      ? this.repos.filter(
          (r) =>
            (params.provider === Provider.NEWSAPI &&
              r instanceof NewsRepositoryImpl) ||
            (params.provider === Provider.GUARDIAN &&
              r instanceof GuardianRepositoryImpl) ||
            (params.provider === Provider.NYTIMES &&
              r instanceof NytRepositoryImpl),
        )
      : this.repos;
    ...
    const successes = settled.filter(
      (r): r is PromiseFulfilledResult<ArticleCollection> =>
        r.status === "fulfilled",
    );
    ...
    const totalResults = successes.reduce(
      (sum, r) => sum + r.value.totalResults,
      0,
    );
